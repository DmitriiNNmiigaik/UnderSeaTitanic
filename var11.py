...
def filter_by_age_category(data, category):
    ...
    filtered_data = [rov for rov in data if min_age <= rov['Age'] < max_age]
...
def do_var11():
    st.image('titanik_2.jpg')
    st.title('Анализ погибших и спасенных пассажиров Титаника разных'
    ...
    if st.button('Показать результаты'):
        ...
        st.write(f"Доля спасенных пассажиров ({age_category}):"
                 f"{survived_rate:.2%}")
        st.write(f"Доля погибших пассажиров ({age_category}):"
                 f"{deceased_rate:.2%}")
