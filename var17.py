...
def get_bilet(lines):
    ...
    for line in lines:
        ...
        if data[12].strip() == 'S':     # strip убирает переход - enter
            ...
            if (S_min > float(data[10]) or S_min == -1) \
                    and float(data[10]) != 0:
                S_min = float(data[10])
        ...
        if data[12].strip() == 'C':     # strip убирает переход - enter
            ...
            if (C_min > float(data[10]) or C_min == -1) \
                    and float(data[10]) != 0:
                C_min = float(data[10])
        ...
        if data[12].strip() == 'Q':     # strip убирает переход - enter
            ...
            if (Q_min > float(data[10]) or Q_min == -1) \
                    and float(data[10]) != 0:
                Q_min = float(data[10])
    ...
    return (
        round(S_bilet / S_passajir, 2), S_min, S_max,
        round(C_bilet / C_passajir, 2), C_min, C_max,
        round(Q_bilet / Q_passajir, 2), Q_min, Q_max
    )
...
with open("data.csv") as file:
    ...
    (
        S_srednee, S_min, S_max,
        C_srednee, C_min, C_max,
        Q_srednee, Q_min, Q_max
    ) = get_bilet(lines)
